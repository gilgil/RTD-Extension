...
addin_id = "org.tcptortd.getinfo"
addin_version = "1.0.0"
addin_displayname = "RTD Extension"
addin_publisher_link = "https://github.com/gilgil/RTD-Extension"
...
add_manifest_entry(manifest_xml, 'uno-typelibrary;type=RDB', 'XTCPToRTD.rdb')
add_manifest_entry(manifest_xml, 'configuration-data', 'TCPToRTD.xcu')
add_manifest_entry(manifest_xml, 'uno-component;type=Python', 'TCPToRTD.py')
manifest_xml.write('</manifest:manifest> \n')
...
def define_function(xml_file, function_name, description, parameters):
    ...
    xml_file.write(
        '    <prop oor:name="CompatibilityName"><value xml:lang="en">AutoAddIn.TCPToRTD.' + function_name + '</value></prop>\n')
...
implementation_name = "org.tcptortd.getinfo.python.TCPToRTDImpl"

# Name of the corresponding Excel add-in if you want to share documents across OOo and Excel.
excel_addin_name = "TCPToRTD.xlam"

tcptortd_xml = open(cur_dir + '/build/TCPToRTD.xcu', 'w')

tcptortd_xml.write('<?xml version="1.0" encoding="UTF-8"?>\n')
tcptortd_xml.write(
...
tcptortd_xml.write('<node oor:name="AddInInfo">\n')
tcptortd_xml.write('<node oor:name="' + implementation_name + '" oor:op="replace">\n')
tcptortd_xml.write('<node oor:name="AddInFunctions">\n')

define_function(tcptortd_xml,
                'RTD', 'Fetches Realtime Data.',
                [('key', 'The key for the data field to fetch.')])

tcptortd_xml.write('</node>\n')
tcptortd_xml.write('</node>\n')
tcptortd_xml.write('</node>\n')
tcptortd_xml.write('</oor:component-data>\n')
